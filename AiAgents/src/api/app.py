...
class ChatResponse(BaseModel):
    ...
    # Hallucination detection transparency fields
    confidence: Optional[float] = None       # 0.0–1.0, from grounding check
    was_recovered: bool = False              # True if answer was re-generated after detection
    recovery_reason: Optional[str] = None   # Why recovery was triggered
...
async def chat(req: ChatRequest):
    ...
    try:
        ...
        # ── Hallucination Guard (post-generation) ────────────────────────────
        # Runs the async tiered cascade: semantic similarity → LLM judge → claim decomp
        # Only runs when retrieved context is available (RAG / GraphRAG agents)
        ...
        retrieved_context = (
            ...
            or final_state.get("metadata", {}).get("retrieved_context", "")
        )

        if retrieved_context and answer:
            try:
                from src.guardrails.hallucination_detector import run_hallucination_cascade
                from src.guardrails.hallucination_recovery import recover_from_hallucination
                from src.models.schemas import HallucinationVerdict
                from src.guardrails.safety_config import DEFAULT_SAFETY_CONFIG

                hall_result = await run_hallucination_cascade(
                    answer=answer,
                ...
                logger.info(
                    f"[{session_id}] Hallucination check: verdict={hall_result.verdict.value} "
                    f"risk={hall_result.risk_score} tiers={hall_result.detection_tiers_run}"
                )

                # Set confidence from grounding result
                if hall_result.semantic_similarity is not None:
                    confidence = round(hall_result.semantic_similarity, 2)
                elif hall_result.llm_judge_confidence is not None:
                    confidence = round(hall_result.llm_judge_confidence, 2)

                # Trigger recovery if hallucination detected
                if hall_result.verdict != HallucinationVerdict.FAITHFUL and hall_result.retry_suggested:
                    logger.warning(
                        f"[{session_id}] Hallucination detected ({hall_result.verdict.value}) — "
                        f"starting recovery. Unsupported claims: {hall_result.unsupported_claims}"
                    )
                    recovery = await recover_from_hallucination(
                        original_answer=answer,
                        context=retrieved_context,
                        user_query=req.message,
                        hallucination_result=hall_result,
                        attempt=0,
                        max_attempts=DEFAULT_SAFETY_CONFIG.max_hallucination_retries,
                    )
                    answer = recovery.answer
                    was_recovered = recovery.was_recovered
                    recovery_reason = recovery.recovery_reason
                    if recovery.confidence > 0:
                        confidence = recovery.confidence
                    logger.info(
                        f"[{session_id}] Recovery complete: strategy={recovery.strategy_used} "
                        f"confidence={confidence}"
                    )

            except Exception as e:
                logger.error(
                    f"[{session_id}] Hallucination guard failed (non-fatal): {e}",
                    exc_info=True,
                )
                # Never fail the request because of a guardrail error

        # ── Output safety check (toxicity, PII in response) ──────────────────
        try:
            from src.guardrails.output_validator import validate_output
            from src.models.schemas import GuardrailDecision
            output_result = validate_output(
                text=answer,
                context=retrieved_context,
                block_toxic=True,
                block_high_hallucination=False,  # async cascade above already handles this
            )
            if output_result.decision == GuardrailDecision.BLOCK:
                logger.warning(f"[{session_id}] Output blocked by toxicity guard")
                answer = (
                    "I'm sorry, but I'm unable to provide that response due to content policy. "
                    "Please rephrase your question."
                )
        except Exception as e:
            logger.error(f"[{session_id}] Output validator failed (non-fatal): {e}")
